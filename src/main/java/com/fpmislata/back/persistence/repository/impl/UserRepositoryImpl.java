...
public class UserRepositoryImpl implements UserRepository {
  ...
  public List<UserEntity> findByName(String name) {
    List<UserJpaEntity> jpaEntities = userDao.findByName(name).stream().toList();
  ...
  public List<UserEntity> findByEmail(String email) {
    List<UserJpaEntity> jpaEntities = userDao.findByEmail(email).stream().toList();
    if (jpaEntities.isEmpty()) {
      return List.of();
    }
    return jpaEntities.stream()
  ...
  @Override
  public UserEntity logByEmail(String email) {
    List<UserJpaEntity> jpaEntities = userDao.findByEmail(email).stream().toList();
    if (jpaEntities.isEmpty()) {
      return null;
    }
